...
- ✅ **Transiciones de estado por botones (sin select)**: `tomar_ticket` pasa PENDIENTE/REABIERTO → EN_PROCESO y guarda el estado previo en `Ticket.estado_previo`; **`liberar_ticket` (nuevo)**: un dev/coor marca su `TicketDesarrollador.activo=False` (conservando histórico); si era el último colaborador activo, el ticket vuelve a `estado_previo` (PENDIENTE o REABIERTO) y lo limpia. `cambiar_estado_ticket` solo permite EN_PROCESO→CERRADO (**dueño o colaborador activo** — un dev que liberó ya NO puede cerrar; `_puede_cerrar` en el context + verificación server-side) y CERRADO→REABIERTO (cualquiera); al cerrar limpia `estado_previo`. Setea/limpia `cerrado_en`. Modelo: `Ticket.estado_previo` + `TicketDesarrollador.activo` (migración `0003`). En el detalle la tarjeta de acciones muestra **Cerrar** (dueño o colaborador activo vía `puede_cerrar`) y **Liberar** (solo si `puede_liberar`, o sea colaborador activo ➜ sustituyó al viejo "Volver a pendiente") y **Tomar ticket** (dev que NO es colaborador activo — tanto en EN_PROCESO como en PENDIENTE/REABIERTO; NO se muestra si ya colabora activamente, p.ej. tras reabrir un ticket que tomó, así no "re-toma" y `_puede_actuar` igualmente lo oculta → llama a `ticket_tomar`, que agrega/activa como colaborador; en EN_PROCESO mantiene EN_PROCESO y sin tocar `estado_previo`); la sección Colaboradores tacha/marca "Liberado" a los `activo=False`. Colores de estado (`estado_badge.html`): PENDIENTE=rojo, EN_PROCESO=verde, CERRADO=gris oscuro, REABIERTO=azul brand
...
- ✅ **BUG — reabrir sin botones de acción RESUELTO + regla de toma**: escenario PENDIENTE → tomar → cerrar → reabrir. Un dev que ya es colaborador activo (lo tiene tomado) **NO vuelve a "tomar"** y **NO ve el botón Tomar** (sería contradictorio con que aparece también Liberar). En **REABIERTO** el panel se comporta como un ticket **EN_PROCESO tomado**: dev colaborador activo ve **Cerrar + Liberar** (se habilitó la transición REABIERTO→CERRADO para colaborador activo/dueño en `cambiar_estado_ticket`); dev NO colaborador ve solo **Tomar ticket** (→ EN_PROCESO). En PENDIENTE el no-colaborador ve Tomar. `_puede_actuar`: REABIERTO → dev cualquiera True, coord/dueño solo si colabora activamente. Verificado con test client (dev activo y dev nuevo).
