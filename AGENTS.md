...
- ❌ Pendiente: **BUG — reabrir sin botones de acción**: escenario PENDIENTE → tomar → cerrar → reabrir. El dev que tomó sigue siendo colaborador activo, pero en el detalle REABIERTO no ve ni "Cerrar", ni "Liberar", ni "Tomar" (la tarjeta queda vacía u oculta). **Comportamiento esperado**: el que reabre y ya era colaborador **saltea el paso de tomar ticket** (conserva su colaboración de antes de cerrar) pero SÍ debe poder cerrar/liberar. No debe ocultarse "Tomar ticket" para quien NO es colaborador (en un REABIERTO eso está bien); el bug es que el colaborador activo reabierto se queda sin ninguna acción. Ver además `_puede_actuar` para REABIERTO.
